...
async function sendViaResend(
  to: string,
  rendered: ReturnType<typeof renderOrderConfirmationEmail>,
...
function logEmailPreview(to: string, rendered: ReturnType<typeof renderOrderConfirmationEmail>) {
  console.info("[mbody] Order confirmation email (log mode)");
  console.info(`  to: ${to}`);
  console.info(`  subject: ${rendered.subject}`);
  console.info(`  preview: ${publicEnv.appUrl}/api/dev/emails/order-confirmation`);
}

/** Sends order confirmation after successful payment (Phase 4 webhook). */
...
export { renderOrderConfirmationEmail };
